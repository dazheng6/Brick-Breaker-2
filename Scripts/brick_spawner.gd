...
func spawn():
	...
	var spawn_position_x = ((-row_width + brick_size.x + margin.x) / 2) + 130
	...
	for i in ROWS:
		for j in COLUMNS:
			...
			add_child(brick)
			var x = spawn_position_x + j * (margin.x + brick.get_size().x ) + 400
			var y = spawn_position_y + i * (margin.y + brick.get_size().y ) + 250
